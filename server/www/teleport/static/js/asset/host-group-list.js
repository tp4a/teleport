...
$app.create_controls = function (cb_stack) {
    ...
    var table_groups_options = {
        ...
        data_source: {
            type: 'ajax-post',
            url: '/asset/get-host-groups-with-member'
        },
    ...
    $tp.create_table_pagination($app.table_groups, 'table-groups-pagination');
...
$app.on_table_groups_render_created = function (render) {
    ...
    render.group_info = function (row_id, fields) {
        return '<a href="/asset/host-group/' + fields.id + '">' + fields.name + '</a><div class="group-desc">' + fields.desc + '</div>'
    ...
    render.members = function (row_id, fields) {
        ...
        for (var i = 0; i < fields.members.length; ++i) {
            ...
            if (fields.members[i].router_ip.length > 0)
                ...
                ret.push('"');
            ret.push('>');
            if (fields.members[i].name.length === 0)
                ret.push(fields.members[i].ip);
            else
                ret.push(fields.members[i].name + ' <span class="mono">[' + fields.members[i].ip + ']</span>');
            ret.push('</div></div>');
        }

        if (fields.member_count > 5) {
            ret.push('<div class="acc-info-wrap"><div class="acc-info">');
            ret.push('<a href="/asset/host-group/' + fields.id + '">...更多 <i class="fa fa-arrow-circle-right"></i></a>');
            ret.push('</div></div>');
        }
...
$app.on_btn_lock_group_click = function (_row_id) {
    ...
    $tp.ajax_post_json('/group/lock', {gtype: TP_GROUP_HOST, glist: group_list},
        function (ret) {
            if (ret.code === TPE_OK) {
                $app.table_groups.load_data();
...
$app.on_btn_unlock_group_click = function (_row_id) {
    ...
    $tp.ajax_post_json('/group/unlock', {gtype: TP_GROUP_HOST, glist: group_list},
        function (ret) {
            if (ret.code === TPE_OK) {
                $app.table_groups.load_data();
...
$app.on_btn_remove_group_click = function (_row_id) {
    ...
    var _fn_sure = function (cb_stack, cb_args) {
        $tp.ajax_post_json('/group/remove', {gtype: TP_GROUP_HOST, glist: group_list},
            function (ret) {
                if (ret.code === TPE_OK) {
                    $app.table_groups.load_data();
                    $tp.notify_success('删除分组操作成功！');
                } else {
                    $tp.notify_error('删除分组操作失败：' + tp_error_msg(ret.code, ret.message));
                }

                cb_stack.exec();
            },
            function () {
                $tp.notify_error('网络故障，删除分组操作失败！');
                cb_stack.exec();
            }
        );
    };
...
$app.create_dlg_edit_group = function () {
    ...
    dlg.on_save = function () {
        ...
        $tp.ajax_post_json('/group/update', {
            ...
            function (ret) {
                if (ret.code === TPE_OK) {
                    $tp.notify_success('主机分组' + action + '成功！');
                    $app.table_groups.load_data();
                    dlg.dom.dialog.modal('hide');
                } else {
                    $tp.notify_error('主机分组' + action + '失败：' + tp_error_msg(ret.code, ret.message));
                }
            },
            function () {
                $tp.notify_error('网络故障，主机分组' + action + '失败！');
            }
        );
