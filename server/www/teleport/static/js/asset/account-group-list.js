...
$app.create_controls = function (cb_stack) {
    ...
    var table_groups_options = {
        ...
        data_source: {
            type: 'ajax-post',
            url: '/asset/get-account-groups-with-member'
        },
...
$app.on_table_groups_render_created = function (render) {
    ...
    render.group_info = function (row_id, fields) {
        return '<a href="/asset/account-group/' + fields.id + '">' + fields.name + '</a><div class="group-desc">' + fields.desc + '</div>'
    ...
    render.members = function (row_id, fields) {
        ...
        if (fields.member_count > 5) {
            ret.push('<div class="acc-info-wrap"><div class="acc-info">');
            ret.push('<a href="/asset/account-group/' + fields.id + '">...更多 <i class="fa fa-arrow-circle-right"></i></a>');
            ret.push('</div></div>');
        }
...
$app.on_btn_remove_group_click = function (_row_id) {
    ...
    var _fn_sure = function (cb_stack, cb_args) {
        $tp.ajax_post_json('/group/remove', {gtype: TP_GROUP_ACCOUNT, glist: group_list},
            function (ret) {
                if (ret.code === TPE_OK) {
                    $app.table_groups.load_data();
...
$app.create_dlg_edit_group = function () {
    ...
    dlg.on_save = function () {
        ...
        $tp.ajax_post_json('/group/update', {
            ...
            function (ret) {
                if (ret.code === TPE_OK) {
                    $tp.notify_success('账号分组' + action + '成功！');
                    $app.table_groups.load_data();
                    dlg.dom.dialog.modal('hide');
                } else {
                    $tp.notify_error('账号分组' + action + '失败：' + tp_error_msg(ret.code, ret.message));
                }
            },
            function () {
                $tp.notify_error('网络故障，账号分组' + action + '失败！');
            }
        );
